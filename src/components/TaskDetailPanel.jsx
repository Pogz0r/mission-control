import { ArrowRight, Clock3, FilePenLine, GripVertical, Hash, Link2, MessageSquareText, UserRound, X } from 'lucide-react'
      ...
      <div className="flex h-full w-full max-w-xl flex-col border-l border-zinc-800 bg-zinc-950 shadow-2xl">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
          <div className="flex flex-wrap gap-2">
            <span className={`rounded-full px-3 py-1.5 text-xs ring-1 ${statusTone[task.status] || 'bg-zinc-800 text-zinc-300 ring-zinc-700'}`}>{task.status}</span>
          ...
          <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-5 shadow-glow">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold text-white">Discord references</h3>
                <p className="mt-1 text-sm text-zinc-500">Track where this task came from inside Discord.</p>
              </div>
              <Hash className="h-4 w-4 text-zinc-500" />
            </div>
            <div className="mt-4 space-y-3 text-sm text-zinc-300">
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">Channel</div>
                <div className="mt-2">{task.discordChannel || 'No channel saved'}</div>
              </div>
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">Thread / message</div>
                <div className="mt-2">{task.discordThread || task.discordMessage || 'No thread/message saved'}</div>
                {task.discordMessageUrl ? (
                  <a href={task.discordMessageUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-sky-300 hover:text-sky-200">
              ...
              {(task.refs || []).length ? (
                <div className="space-y-2">
                  {(task.refs || []).map((ref, index) => (
                    <div key={`${ref.label || ref.url || index}`} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                      <div className="text-sm font-medium text-white">{ref.label || `Reference ${index + 1}`}</div>
                      <div className="mt-2 text-zinc-400">{ref.note || ref.url || 'No note'}</div>
                      {ref.url ? (
                        <a href={ref.url} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-sky-300 hover:text-sky-200">
                          <Link2 className="h-4 w-4" />
                          Open reference
                        </a>
                      ) : null}
          ...
          <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-5 shadow-glow">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {['Today', 'In Progress', 'Waiting', 'Done'].map((status) => (
                <button key={status} onClick={() => onMove(status)} className={`rounded-2xl px-3 py-2 text-sm transition ${task.status === status ? 'bg-violet-500/15 text-violet-300 ring-1 ring-violet-500/20' : 'bg-zinc-950 text-zinc-300 ring-1 ring-zinc-800 hover:bg-zinc-800'}`}>
                  {status}
                </button>
              ))}
            </div>
          ...
          <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-5 shadow-glow">
            ...
            {task.tags?.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {task.tags.map((tag) => (
                  <span key={tag} className="rounded-full bg-zinc-950 px-3 py-1.5 text-xs text-zinc-300 ring-1 ring-zinc-800">{tag}</span>
                ))}
              </div>
            ) : null}
          </section>
        ...
        <div className="flex items-center justify-between border-t border-zinc-800 px-6 py-5">
          ...
          <button onClick={onEdit} className="inline-flex items-center gap-2 rounded-2xl bg-violet-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-violet-400">
